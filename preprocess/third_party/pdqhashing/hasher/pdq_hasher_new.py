...
class PDQHasher:
    ...
    def compute_dct_matrix(self):
        matrix_scale_factor = math.sqrt(2.0 / float(self.DOWNSAMPLE_SIZE))
        d = [0] * self.HASH_SIZE
        for i in range(0, self.HASH_SIZE):
            di = [0] * self.DOWNSAMPLE_SIZE 
            for j in range(0, self.DOWNSAMPLE_SIZE):
                di[j] = math.cos((math.pi / 2 / float(self.DOWNSAMPLE_SIZE)) * (i + 1) * (2 * j + 1))
            d[i] = di
        return d

    def __init__(self, downsample_size=64, hash_size=16) -> None:
        ...
        self.DCT_MATRIX_SCALE_FACTOR = float(math.sqrt(2.0 / float(self.DOWNSAMPLE_SIZE)))
        ...
        self.PDQ_JAROSZ_WINDOW_SIZE_DIVISOR = self.DOWNSAMPLE_SIZE * 2 
    ...
    def fromFile(self, filepath, hashingMetadata=None):
        ...
        buffer64x64 = MatrixUtil.allocateMatrix(self.DOWNSAMPLE_SIZE, self.DOWNSAMPLE_SIZE)
        buffer16x64 = MatrixUtil.allocateMatrix(self.HASH_SIZE, self.DOWNSAMPLE_SIZE)
        buffer16x16 = MatrixUtil.allocateMatrix(self.HASH_SIZE, self.HASH_SIZE)
        t1 = time.time()
        rv = self.fromImage(
    ...
    def dihedralFromFile(self, filename, hashingMetadata, dihFlags):
        ...
        buffer16x64 = MatrixUtil.allocateMatrix(self.HASH_SIZE, self.DOWNSAMPLE_SIZE)
        buffer16x16 = MatrixUtil.allocateMatrix(self.HASH_SIZE, self.HASH_SIZE)
        buffer16x16Aux = MatrixUtil.allocateMatrix(self.HASH_SIZE, self.HASH_SIZE)
        t1 = time.time()
    ...
    #@classmethod
    def decimateFloat(
        self, in_, inNumRows, inNumCols, out  # numRows x numCols in row-major order
    ):
        for i in range(self.DOWNSAMPLE_SIZE):
            ini = int(((i + 0.5) * inNumRows) / self.DOWNSAMPLE_SIZE)
            for j in range(self.DOWNSAMPLE_SIZE):
                inj = int(((j + 0.5) * inNumCols) / self.DOWNSAMPLE_SIZE)
                out[i][j] = in_[ini * inNumCols + inj]

    #@classmethod
    def computePDQImageDomainQualityMetric(self, buffer64x64):
        ...
        """
        gradientSum = 0
        for i in range(self.DOWNSAMPLE_SIZE-1):
            for j in range(self.DOWNSAMPLE_SIZE):
                u = buffer64x64[i][j]
                v = buffer64x64[i + 1][j]
                d = int(((u - v) * 100) / 255)
        ...
        for i in range(self.DOWNSAMPLE_SIZE):
            for j in range(self.DOWNSAMPLE_SIZE-1):
                u = buffer64x64[i][j]
                v = buffer64x64[i][j + 1]
                d = int(((u - v) * 100) / 255)
                gradientSum += int(abs(d))
    ...
    def dct64To16(self, A, T, B):
        ...
        T = [0] * self.HASH_SIZE
        for i in range(0, self.HASH_SIZE):
            ti = [0] * self.DOWNSAMPLE_SIZE 
            for j in range(0, self.DOWNSAMPLE_SIZE):
                tij = 0.0
                for k in range(0, self.DOWNSAMPLE_SIZE):
                    tij += D[i][k] * A[k][j]
                ti[j] = tij
            T[i] = ti
        ...
        for i in range(self.HASH_SIZE):
            for j in range(self.HASH_SIZE):
                sumk = float(0.0)
                for k in range(self.DOWNSAMPLE_SIZE):
                    sumk += T[i][k] * D[j][k]
                B[i][j] = sumk
    ...
    def pdqBuffer16x16ToBits(self, dctOutput16x16):
        ...
        """
        hash = Hash256()
        dctMedian = MatrixUtil.torben(dctOutput16x16, self.HASH_SIZE, self.HASH_SIZE)
        for i in range(self.HASH_SIZE):
            for j in range(self.HASH_SIZE):
                if dctOutput16x16[i][j] > dctMedian:
                    hash.setBit(i * self.HASH_SIZE + j)
        return hash

    #@classmethod
    def computeJaroszFilterWindowSize(self, dimension):
        """ Round up. See comments at top of file for details. """
        return int(
            (dimension + self.PDQ_JAROSZ_WINDOW_SIZE_DIVISOR - 1)
